...
public class DiscountController{
    ...
    public Page<Discount> getAll(PageQuery pageQuery, HttpServletRequest request) {
        ...
        Integer discounttypeId = pageQuery.getSearchParamAsInteger("discounttype");
        Integer discountstatusId = pageQuery.getSearchParamAsInteger("discountstatus");
        ...
        List<Discount> filteredDiscounts = stream.filter(discount -> {
            ...
            if(discounttypeId!=null)
                if(!discount.getDiscounttype().getId().equals(discounttypeId)) return false;
            if(discountstatusId!=null)
                if(!discount.getDiscountstatus().getId().equals(discountstatusId)) return false;
            return true;
    ...
    public Page<Discount> getAllBasic(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all discounts' basic data", UsecaseList.SHOW_ALL_DISCOUNTS, UsecaseList.ADD_ENROLLMENT, UsecaseList.UPDATE_ENROLLMENT);
    ...
    public Discount get(@PathVariable Integer id, HttpServletRequest request) {
        accessControlManager.authorize(request, "No privilege to get discount", UsecaseList.SHOW_DISCOUNT_DETAILS, UsecaseList.UPDATE_DISCOUNT);
    ...
    public ResourceLink add(@RequestBody Discount discount, HttpServletRequest request) throws InterruptedException {
        ...
        discount.setId(null);
        discount.setDiscountstatus(new Discountstatus(1));;
