...
export class UserFormComponent extends AbstractComponent implements OnInit {
  ...
  loadData(): any {
    ...
    this.userService.getAllNonUserGuardians().then((data) => {
      this.guardians = data;
    }).catch((e) => {
      console.log(e);
      this.snackBar.open('Something is wrong', null, {duration: 2000});
    ...
    this.userService.getAllNonUserStudents().then((data) => {
      this.students = data;
    }).catch((e) => {
      console.log(e);
      this.snackBar.open('Something is wrong', null, {duration: 2000});
  ...
  async submit(): Promise<void> {
    ...
    user.employee = this.typeField.value === 'employee' ? this.employeeField.value : null;
    user.guardian = this.typeField.value === 'guardian' ? this.guardianField.value : null;
    user.student = this.typeField.value === 'student' ? this.studentField.value : null;
    ...
    try{
      ...
      switch (e.status) {
        ...
        case 400:
          ...
          if (msg.employee || msg.guardian || msg.student || msg.password){
            if (msg.employee) { this.employeeField.setErrors({server: msg.employee}); }
            if (msg.guardian) { this.guardianField.setErrors({server: msg.guardian}); }
            if (msg.student) { this.studentField.setErrors({server: msg.student}); }
